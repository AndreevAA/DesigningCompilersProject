...
def IsParam(obj):
    return ((obj.m_value.m_class == OSG.PAR.getValue()) or (obj.m_value.m_class == OSG.VAR.getValue()) and (
                obj.m_value.m_val > 0))
...
def StatSequence():
    ...
    while True:
        ...
        if sym.m_value == OSS.SEMICOLON.getValue():
            OSS.Get(sym)
        elif (sym.m_value >= OSS.SEMICOLON.getValue()) and (sym.m_value < OSS.IF.getValue()) or (
                sym.m_value >= OSS.ARRAY.getValue()):
            break
        else:
            OSS.Mark("; ?")
